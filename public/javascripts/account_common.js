...
$.extend($.fn, {
	validate: function( options ) {
		...
		// 要素に紐づくデータが既にある場合は、データを返す
		var validator = $.data(this[0], 'validator');
		if ( validator ) {
			return validator;
		}
		
		// validatorオブジェクトを生成し、要素にデータ設定する
		validator = new $.validator( options, this[0] );
		$.data(this[0], 'validator', validator); 

		// 送信時チェックの場合(default: true)
		if ( validator.settings.onsubmit ) {
		
			// キャンセルボタンにclickイベントを設定
			...
			// sumitHandlerの設定がある場合は、キャプチャーする
			// when a submitHandler is used, capture the submitting button
			if (validator.settings.submitHandler) {
				// submitボタンにclickイベントを設定
			...
			this.submit( function( event ) {
				...
				function handle() {
					// submitHandler設定がある場合
					if ( validator.settings.submitHandler ) {
						// 送信ボタンがある場合
						if (validator.submitButton) {
							// insert a hidden input as a replacement for the missing submit button
							// hiddenオブジェクトに、送信ボタンのname, valueを設定し、フォームに追加する
				...
				// formチェックを実行して、結果がOKの場合、送信処理を行う
				if ( validator.form() ) {
					// pendingになっている要求がある場合
...
$.extend($.validator, {
	...
	prototype: {
		...
		check: function( element ) {
			...
			// ラジオボタン、チェックボックスの場合、最初の要素をチェックする
		...
		/**
		 * @description 携帯メールアドレスかどうかチェックする
		 **/
		checkMobileEmail : function(value){
			var sValue = this.getStr(value);
			...
			var oMobile = new RegExp("^("
			+ [
				"docomo\.ne\.jp",
				"ezweb\.ne\.jp",
				"softbank\.ne\.jp",
				"([a-z])\.vodafone\.ne\.jp",
				"(.*\.?|)sky\.(tu-ka|tkk|tkc)\.ne\.jp",
				"(d(.?)\.|wm\.|)?pdx\.ne\.jp",
				"disney\.ne\.jp",
				"emnet\.ne\.jp",
				"(.*\.|)mnx\.ne\.jp",
...
*/
		email: function(value, element) {
			return this.optional(element) || /^[\w!#$&'*+/=?^_`{|}.-]+[@|＠]([a-zA-Z0-9]([\w-]*[a-zA-Z0-9])?\.)*[a-zA-Z0-9][\w-]*[a-zA-Z0-9]\.[a-zA-Z]{2,4}$/i.test(value);
		},
		nomobile: function(value, element){
			return this.optional(element) || !this.checkMobileEmail($.trim(value));
		},
	...
	nj.account.validate.prototype = {
		...
		setFocusEvent : function(){
			...
			// ユーザ名のみマイページURLを表示する
			    ...
			    // 背景を変更する
		        $(this).closest(".MdInputTxt01").addClass("mdInputTxt01Active");   //tao 9.25
		        $(this).closest(".MdInputTxt03").addClass("mdInputTxt03Active");
				$(this).closest(".MdInput01").addClass("mdInput01Active");
		    });
		},
		// textboxのfocusが外れた場合、非activeにする
		setBlurEvent : function(){
		    $("input:text, input:password").blur(function(){
		        $(this).closest(".MdInputTxt01").removeClass("mdInputTxt01Active");
		        $(this).closest(".MdInputTxt03").removeClass("mdInputTxt03Active");
				$(this).closest(".MdInput01").removeClass("mdInput01Active");
		    });
		...
		// captcha更新イベント設定
		setCaptchaEvent : function(target){
			if(!target) return;
		    // captcha更新イベント設定
		    $(target).click(function(){
		        // captcha用のURLを設定
		        $.post("/reloadCaptcha", null, function(oReq){    // here is a post request.
		            var data = oReq;
		            $("#captchaKey").val(data.captchaKey);
		            //$("#captchaImg").attr("src", "https://ssl.naver.jp/" + data.captchaImg);
                    // revise todo
					if($("#_captcha")){
						$("#_captcha").val("");
					}
...
/**
 * アカウント削除ページ
 * https://ssl.naver.jp/withdraw/form
 **/
// what does this function used for 9.25 tao
(function($){
	nj.account.initAccountDeleteForm = function(){
		// パスワード変換は、iphone, ipad, android以外で行う(NJUSER-3303)
		var ua = navigator.userAgent;
		if(!/iphone/i.test(ua) && !/ipad/i.test(ua) && !/android/i.test(ua)){
			this.setPasswdCheck();
		}
		this.setFocusEvent();
		this.setBlurEvent();
	...
	$.extend(nj.account.initAccountDeleteForm.prototype, nj.account.validate.prototype, {
		setTextAreaFocus : function(){
			$("#_opinion").focus(function(){
				$(this).closest(".MdTextarea01").addClass("mdTextarea01Active");
			});
			$("#_opinion").blur(function(){
				$(this).closest(".MdTextarea01").removeClass("mdTextarea01Active");
			});
		},
...
/**
 * インセンティブ情報変更ページ
 * https://ssl.naver.jp/accountEmailBank
 **/
(function($){
	nj.account.initIncentiveInfoChangeForm = function(){
		this.setInitClass();
		this.setRadioEvent();
		this.setFocusEvent();
		this.setBlurEvent();
	}
	$.extend(nj.account.initIncentiveInfoChangeForm.prototype, nj.account.validate.prototype, {
		setInitClass : function(){
			$("input:text").each(function(){
				var target = $(this);
				if(target.attr("placeholder") && target.val()){
					target.closest("SPAN.MdInput01").addClass("mdInput01Blur");
				}
			});		
		},
		setRadioEvent : function(){
			if($('input[name="corporateType"]:checked').attr("id") == "radio02"){
				var target = $("#ipt_bank_2");
				target.attr("disabled", "disabled");
				target.closest("SPAN.MdInput01").addClass("mdInput01Disabled");
				target.val("");
				target.attr("placeholder", "")
			}
			$('input[name="corporateType"]:radio').change(function(){
				var target = $("#ipt_bank_2");
				if($(this).attr("id") == "radio02"){
					...
					target.val("");
					target.attr("placeholder", "")
					target.closest("SPAN.MdInput01").next("SPAN.mdSting01Err01").remove();
				}else{
					target.val("名");
					target.attr("placeholder", "名");
					target.closest("SPAN.MdInput01").addClass("mdInput01Blur");
					target.attr("disabled", "");
					target.closest("SPAN.MdInput01").removeClass("mdInput01Disabled");
				}
			});
		},
		setFocusEvent : function(){
			$("input:text").focus(function(){
				...
				target.closest("SPAN.MdInput01").removeClass("mdInput01Blur");
				$(this).closest(".MdInput01").addClass("mdInput01Active");
			});		
		},
		setBlurEvent : function(){
...
/**
 * その他情報変更ページ
 * https://ssl.naver.jp/accountEtc
 **/
(function($){
	nj.account.initOtherInfoChangeForm = function(){
		this.setSelectMenu();
	}
	$.extend(nj.account.initOtherInfoChangeForm.prototype, nj.account.validate.prototype, {
		setSelectMenu : function(){
			$('.MdSelect01 select').selectmenu({
				style:'dropdown',
				width:'146px',
				open : function(){
					if($.browser.mozilla){
...
(function($){
	nj.account.initPasswdResetMailForm = function(){
		this.setFocusEvent();
		this.setBlurEvent();
		this.setCaptchaEvent("SPAN.mdCaptcha02ReloadTxt");
	}
...
/**
 * opendidからのログイン画面
 * https://ssl.naver.jp/openid/join
 **/
(function($){
	nj.account.initRegistOpenIdForm = function(){
		this.setKeyupEvent();
		this.setUserUrl();
		this.setFocusEvent();
		this.setBlurEvent();
		this.setClickEvent();
		this.setCaptchaEvent("SPAN.mdCaptcha01ReloadTxt");
		this.setSubmitEvent();
	};
	$.extend(nj.account.initRegistOpenIdForm.prototype, nj.account.validate.prototype, {});
})(jQuery);
/**
 * 登録画面
...
(function($){
	nj.account.initRegistForm = function(){
		...
		//this.setCaptchaEvent("SPAN.mdCaptcha01ReloadTxt"); //9.24
		this.setSubmitEvent();
	};
	$.extend(nj.account.initRegistForm.prototype, nj.account.validate.prototype, {
		...
		setFocusEvent : function(){
		    ...
		    $("input:text").focus(function(){
				...
				// 背景を変更する
		        $(this).closest(".MdInputTxt01").addClass("mdInputTxt01Active");
		    });
		},
		setBlurEvent : function(){
		    $("input:text").blur(function(){
 ...
 **/
(function($){
	$.fn.njConfirmMail = function(options){
		if(!this.length){
			return;
		}
		var self = this;
		...
		var defaults = {
			...
			onTimeout   : function(){self.onTimeout();}
		};
		...
		$(this).click(function(){
			...
			$.ajax({
				...
				cache   : false,
				success : self.options.onSuccess,
				error   : self.options.onError,
				timeout : self.options.timeout
			});
			return false;
		});
		this.onSuccess = function(data, dataType){
